...
3. 如果需要申请动态资源并重设数据，则应提供 `FooTest` 的[默认构造函数](../class/class.md#默认构造函数)或[重写](../class/inheritance.md#虚函数) `SetUp()` 方法。
4. 如果需要释放动态资源，则应提供 `FooTest` 的[析构函数](../class/copy_control.md#析构函数)或[重写](../class/inheritance.md#虚函数)  `TearDown()` 方法。
5. 如果需要，定义其他方法。
...
一般形式为 `-D var=value`，其中 `=` 两边没有空格。常用的有（可以组合使用）：
...
在[图形用户界面的 CMake ](https://cmake.org/download/)里，这些变量可以分组显示，查找和修改起来非常方便。
