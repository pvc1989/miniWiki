...
## 抽象数据类型
「类 (class)」机制最基本的用途是在 `int`、`double` 等「内置 (built-in) 类型」之外，创建新的「抽象数据类型 (Abstract Data Type, ADT)」。
...
- client 只依赖于 interface，而不依赖于 implementation：
  ...
  - implementation 发生变化时，不需要重新 *编译 client 的源代码*，而只需要重新 *编译 implementation 的源代码*，并将所得的目标文件 *链接进 client 的目标文件*。
...
### `friend`
定义一个类时，可以用 `friend` 关键词将其他（可见的）类或函数声明为它的 *友元*，从而允许这些友元访问其私有成员。
*友元声明* 不是 *函数声明*。
...
### 其他接口函数
除了公共的方法成员，还可以在类的外部定义接口函数，最典型的是[重载为普通函数的运算符](./operator.md#通常重载为普通函数的运算符)。
如果需要在这些函数的 *实现* 中访问类的私有成员，则应将它们声明为 [`friend`](#`friend`)。

## `static` 成员
「静态 (static) 成员」由一个 *类* 的所有 *对象* 共享，因此不属于其中任何一个对象：

- 静态 *数据* 成员存储于所有对象的外部，不计入对象的大小。
- 静态 *方法* 成员独立于所有对象，形参列表不含 [`this` 指针](#`this`-指针)。

### 访问静态成员
在类的外部，静态成员可以通过紧跟在 *类名* 后面的「作用域 (scope) 运算符」 `::` 来访问，也可以（像非静态成员一样）通过 *对象* 或指向该对象的 *指针* 或 *引用* 来访问。
...
在构造过程中，需要修改数据成员的值，因此构造函数不可以被声明为 [`const`](#`const`-成员函数)。
...
自 C++11 起，允许在形参列表后紧跟 `= default`，以 *显式* 生成该构造函数。
...
```
「初始化列表 (initializer list)」位于 *形参列表* 与 *函数体* 之间，用于对「值初始化的 (value initialize)」数据成员：
...
以下成员必须利用 *初始化列表* 进行初始化：
- 该成员是 `const`。
- 该成员是 *引用* 类型。
- 该成员没有[默认构造函数](#默认构造函数)。

### 委托构造函数
「委托 (delegated) 构造函数」在其[初始化列表](#初始化列表)中 *调用* 另一个构造函数，从而将构造任务 *委托* 给那个构造函数：
```cpp
class Point {
 public:
  Point(const double& x, const double& y) : x_(x), y_(y) { }
  Point() : Point(0.0, 0.0) { }  // 默认构造函数，委托双参数版本进行构造
 private:
  double x_;
  double y_;
};
...
如果需要禁止（通常应该禁止）这种隐式类型转换，只需要在构造函数头部加上关键词 `explicit`：
```cpp
namespace std{
template <
  class T, 
