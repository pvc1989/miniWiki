...
|  L0  |     寄存器     | 4 或 8 字节“词” |        0         |
|  L1  |   L1 缓存器    |   64 字节“块”   |        4         |
|  L2  |   L2 缓存器    |   64 字节“块”   |        10        |
|  L3  |   L3 缓存器    |   64 字节“块”   |        50        |
|  L4  |    主存储器    |    4 KB “页”    |       200        |
|  L5  |    本地磁盘    |      扇区       |     100,000      |
|  L6  | 分布式文件系统 |      文件       |    10,000,000    |
|  L7  |  互联网服务器  |      网页       |  1,000,000,000   |
...
- 块的大小由相邻两级共享，且随等级升高（L 后的数字减小）而减小。
...
各级存储器都需要有某种机制（硬件或软件）来管理缓存，其任务包括划分块、判断是否命中、丢失后替换块。
...
缓存可以按 $E$ 值来分类：
- 若 $E=1$，即每个缓存集合内只有一条缓存线，则称其为“[直接映射的 (direct-mapped)](#direct-mapped)”。
- 若 $E\in(1,C/B)$，则称其为“[$E$ 路集合关联的 ($E$-way set associative)](#set-associative)”。
- 若 $E=C/B$，即所有缓存线属于同一集合，则称其为“[完全关联的 (fully associative)](#fully-associative)”。

## 直接映射（$E=1$）<a href id="direct-mapped"></a>

### 集合选择

以地址 $A$ 中（从右向左）第 $b$ 位起的 $s$ 位所表示的无符号整数，作为“缓存集合”的序号。
...
若缓存命中，则以地址 $A$ 中最低（右端）的 $b$ 位所表示无符号整数，作为所读数据在当前缓存线内的的偏移量（起始字节编号）。
...
## 集合关联（$1<E<C/B$）<a href id="set-associative"></a>

### 集合选择

同“[直接映射的缓存](#direct-mapped)”之“集合选择”。

### 缓存线匹配、数据选择

每个“缓存集合”可以被视为一个小型“关联存储器 (associative memory)”：

- 【键 (key)】有效位 + 标签位
- 【值 (value)】数据块

当前缓存集合内的任意一条缓存线都可能命中，故缓存器需便历各缓存线。
...
## 完全关联（$E=C/B$）<a href id="fully-associative"></a>

### 集合选择

因 $S=1$ 即 $s=0$，故 $m=t+b$ 即地址只分“缓存线标签”与“数据偏移量”两部分。

### 缓存线匹配、数据选择

同“[集合关联的缓存](#set-associative)”之“缓存线匹配”及“数据选择”。
