...
『总线 (bus)』是连接 CPU 与主存储器或读写设备的共享电路。
- 『系统总线 (system bus)』连接 CPU 中的『总线接口 (bus interface)』与『读写桥 (I/O bridge)』。
- 『存储总线 (memory bus)』连接『读写桥』与『主存储器 (main memory)』。
...
1. 当 DMA 完成后，硬盘控制器向 CPU 发出『中断 (interrupt)』信号。CPU 收到该信号后，暂停执行其他指令。
...
数据以”页面 (page)“为单位进行读写。各页面只有在其所属“区块 (block)”被“擦除 (erase)”后才能写入；被擦除的区块内，各页面可以被写入一次。每个区块大约可以被写入 $10^5$ 次；整块 SSD 大约可以使用十年（连续读写）到几万年（日常读写）。
...
- 第 $k+1$ 级存储器被划分为若干连续的『块 (block)』，各块以地址区分。
- 第 $k$ 级存储器也被划分为同样大小的块，但块的数量少于第 $k+1$ 级。
- 相邻两级存储器通过整块复制，维持上级所含块为下级所含块的子集。
- 块的大小由相邻两级共享，且随等级升高（级别编号减小）而减小。

### 缓存命中

当程序需要从第 $k+1$ 级存储器中读取数据 $d$ 时，存储系统会先在第 $k$ 级存储器中查找。

【缓存命中 (cache hit)】若数据 $d$ 恰好属于第 $k$ 级存储器中的某块，则无需访问第 $k+1$ 级存储器。

### 缓存丢失

【缓存丢失 (cache miss)】若数据 $d$ 不属于第 $k$ 级存储器中的任何块，则需访问第 $k+1$ 级存储器。

访问完第 $k+1$ 级存储器后，可能需要修改第 $k$ 级存储器（替换其中某块的数据）。
...
- 【冷丢失 (cold miss)】第 $k$ 级存储器的所有块均为空。
- 【冲突丢失 (conflict miss)】取自第 $k+1$ 级存储器的不同块，被映射到第 $k$ 级存储器的同一块。
- 【容量丢失 (capacity miss)】存储器容量太小，不足以容纳『工作集 (working set)』。

### 缓存管理

各级存储器都需要有某种（硬件或软件）机制来管理缓存，其任务包括划分块、判断是否命中、丢失后替换块。
...
因 $S=1$ 即 $s=0$，故 $m=t+b$ 即地址只分『缓存线标签』与『块偏移量』两部分。
...
- 【写出分配 (write-allocate)】先从下一级存储器读取相应的块，再更新本级存储器的缓存线。
