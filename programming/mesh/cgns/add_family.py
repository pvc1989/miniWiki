...
def rename(old_file):
    tree, links, paths = cgm.load(old_file)
    base = wrapper.getUniqueChildByType(tree, 'CGNSBase_t')
    families = wrapper.getChildrenByType(base, 'Family_t')
    for family in families:
        family_name = wrapper.getNodeName(family)
        ...
        for child in children:
            if not cgu.checkNodeType(child, 'FamilyName_t'):
                continue
            old_name = wrapper.getNodeName(child)
...
def getFamilyToRoot(families) -> dict:
    family_to_root = dict()
    for family in families:
        children = wrapper.getChildrenByType(family, 'FamilyName_t')
        if 1 == len(children):
            child = children[0]
            family_to_root[wrapper.getNodeName(family)] = cgu.getValueAsString(child)
        elif 0 == len(children):
            name = wrapper.getNodeName(family)
...
def compress(old_file):
    ...
    for zone in zones:
        bc = wrapper.getUniqueChildByType(zone, 'ZoneBC_t')
        bc_list = wrapper.getChildrenByType(bc, 'BC_t')
        for bc in bc_list:
            bc_name = wrapper.getNodeName(bc)
            print(f'In (BC_t) {bc_name}:')
            family = wrapper.getUniqueChildByType(bc, 'FamilyName_t')
