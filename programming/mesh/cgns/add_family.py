...
def getFamilyToRoot(families) -> dict:
    family_to_root = dict()
    for family in families:
        children = getChildrenByType(family, 'FamilyName_t')
        if 1 == len(children):
            child = children[0]
            family_to_root[getNodeName(family)] = cgu.getValueAsString(child)
        elif 0 == len(children):
            name = getNodeName(family)
...
def compress(old_file):
    ...
    for zone in zones:
        bc = getUniqueChildByType(zone, 'ZoneBC_t')
        bc_list = getChildrenByType(bc, 'BC_t')
        for bc in bc_list:
            bc_name = getNodeName(bc)
            print(f'In (BC_t) {bc_name}:')
            family = getUniqueChildByType(bc, 'FamilyName_t')
