...
class Solution {
 public:
  int maxScore(vector<int>& points, int k) {
    auto n = points.size();
    assert(k <= n);
    int prefix_sum = accumulate(points.begin(), points.begin() + k, 0);
    int suffix_sum = 0;
    int score = prefix_sum;
    for (int i = 1; i <= k; ++i) {
      prefix_sum -= points[k - i];
      suffix_sum += points[n - i];
      score = max(score, prefix_sum + suffix_sum);
    }
    return score;
  }
};
