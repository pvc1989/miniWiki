...
class Solution {
  pair<TreeNode*, TreeNode*> build(TreeNode* root) {
    ...
    if (root->right) {
      auto right_result = build(root->right);
      root->right = right_result.first;
      result.second = right_result.second;
    }
    if (root->left) {
      auto left_result = build(root->left);
      root->left = nullptr;
      if (left_result.second) {
        left_result.second->right = root->right;
      }
      root->right = left_result.first;
      if (result.second == root) {
