...
class UndergroundSystem {
  ...
  unordered_map<string, unordered_map<string, Record>> start_and_end_to_record_;
  unordered_map<int, pair<string, int>> id_to_start_;
 ...
 public:
  ...
  void checkIn(int id, string start_station, int start_time) {
    id_to_start_.emplace(id, make_pair(start_station, start_time));
  }

  void checkOut(int id, string end_station, int end_time) {
    auto iter = id_to_start_.find(id);
    auto& [start_station, start_time] = iter->second;
    auto& record = start_and_end_to_record_[start_station][end_station];
    record.total += end_time - start_time;
    record.count += 1;
    id_to_start_.erase(iter);
  }

  double getAverageTime(string start, string end) {
    auto [total, count] = start_and_end_to_record_[start][end];
    assert(count);
    return total / count;
  }
};
