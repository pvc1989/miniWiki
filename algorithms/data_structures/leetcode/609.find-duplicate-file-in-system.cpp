...
class Solution {
 public:
  vector<vector<string>> findDuplicate(vector<string>& paths) {
    auto content_to_files = unordered_map<string, vector<string>>();
    for (auto& info : paths) {
      int head = 0;
      int tail = info.find_first_of(' ');
      auto dir = info.substr(head, tail - head);
      dir.push_back('/');
      head = dir.size();
      while (head < info.size()) {
        tail = info.find_first_of('(', head);
        auto file = info.substr(head, tail - head);
        head = tail + 1;
        tail = info.find_first_of(')', head);
        auto content = info.substr(head, tail - head);
        content_to_files[content].emplace_back(dir + file);
        head = tail + 2;
      }
    }
    auto results = vector<vector<string>>();
    for (auto& [content, files] : content_to_files) {
      if (files.size() > 1) {
        results.emplace_back(move(files));
      }
    }
    return results;
  }
