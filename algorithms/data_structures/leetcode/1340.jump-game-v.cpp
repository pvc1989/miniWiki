...
class Solution {
 public:
  int maxJumps(vector<int> &index_to_height, int max_step) {
    int size = index_to_height.size();
    auto index_to_count = vector<int>(size, 1);
    auto height_to_index_vec = map<int, vector<int>>();
    for (int index = 0; index != size; ++index) {
      height_to_index_vec[index_to_height[index]].emplace_back(index);
    }
    for (auto &[height, index_vec] : height_to_index_vec) {
      // `height`: from lower to higher
      for (auto index : index_vec) {  // for each `index` with this `height`
        int count = 1;
        for (auto step = 1; step <= max_step; ++step) {
          auto another_index = index + step;
          if (another_index < size && index_to_height[another_index] < height) {
            count = max(count, 1 + index_to_count[another_index]);
          } else {
            break;
          }
        }
        for (auto step = 1; step <= max_step; ++step) {
          auto another_index = index - step;
          if (0 <= another_index && index_to_height[another_index] < height) {
            count = max(count, 1 + index_to_count[another_index]);
          } else {
            break;
          }
        ...
        // printf("height, index, count = %d, %d, %d\n", height, index, count);
      }
    }
    return *max_element(index_to_count.begin(), index_to_count.end());
  }
};
// @lc code=end
