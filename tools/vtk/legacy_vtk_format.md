...
一些约定：
- `data_type` 表示数据类型，常用的有 `int`、`long`、`float`、`double`。
- 「关键词 (keyword)」不区分大小写，习惯上全部大写。
- 数组下标从 `0` 开始。
- [DataSet Structure](#DataSet-Structure) 必须出现在 [DataSet Attributes](#DataSet-Attributes) 之前，并且二者的「结点 (node)」及「单元 (cell)」个数必须一致。
...
无论 *数据* 以 `ASCII` 还是 `BINARY` 形式存储， *关键词* 总是以文本形式出现。
...
对于隐含结点连接关系的类型（如 `STRUCTURED_POINTS`、`RECTILINEAR_GRID`），约定 `X` 方向的 *结点编号* 增长得最快，`Y` 方向次之，`Z` 方向最慢。

### `STRUCTURED_POINTS`
该类型对数据「规整性 (regularity)」的要求最高，定义一个对象所需的信息最少，只有 *原点坐标* 和每个方向的 *结点总数* 与 *结点间距* 需要显式给出：
```vtk
...
如果格式要求里没有给出 `data_type`，那么数据类型取决于[文件类型](#File-Format)：
- 如果文件类型为 `ASCII`，则数据为 `[0.0, 1.0]` 内的 `float`。
- 如果文件类型为 `BINARY`，则数据为 `[0, 255]` 内的 `unsigned char`。
...
```
其中 `m` 表示分量个数，因此 `COLOR_SCALARS` 可以看作是 `VECTORS` (`m=3`) 的推广。
